...
var cmd = input.split(/\n/)
var action = cmd.map((cmd) => cmd.split('if'))
  .map((action) => action[0].split(' '))
  .map((action) => ({ v: action[0], d: (action[1] === 'inc' ? '+=' : '-='), b: Number(action[2]) }))
var ifStatement = cmd.map((cmd) => cmd.split('if')).map((i) => ({statement: i[1]}))
var varList = action.map((vars) => ({var: vars.v, value: 0}))
...
function part1 () {
  ...
  for (var i = 0; i < cmd.length; i++) {
    if (eval(ifStatement[i].statement)) {
      if (action[i].d === '+=') {
